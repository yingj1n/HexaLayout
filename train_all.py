...
from mono_model import UNetRoadMapNetwork, UNetRoadMapNetwork_extend, UNetRoadMapNetwork_extend2, Decoder, Encoder
import utils
...
encoder_model_list = [networks.ResnetEncoder(18, False) for i in range(6)]
depth_decoder_model_list = [networks.DepthDecoder(num_ch_enc=encoder_model_list[0].num_ch_enc, scales=range(4)) for i in range(6)]
...
for epoch in range(num_epochs):
    ...
    for batch, (sample, target, road_image, extra) in enumerate(train_loader):
        ...
        train_bscore_list.append(total_ats_bounding_boxes / batch_size)
        ...
        avg_bbox_score = sum(train_bscore_list) / len(train_bscore_list)
        ...
        if batch % 10 == 0:
            print('batch [{}], epoch [{}], loss: {:.4f}, time: {:.0f}s, ts: {:.4f}, bbox_score: {:.4f}'
                  .format(batch + 1, epoch + 1, train_loss / sample_size,
                          batch_time, avg_train_ts, avg_bbox_score))
    ...
    train_bscore = sum(train_bscore_list) / len(train_bscore_list)
    val_ts, val_bscore, predicted_val_map = utils.evaluation_layout(models, encoder_model_list, depth_decoder_model_list, val_loader, DEVICE)
    ...
    print('epoch [{}/{}], loss: {:.4f}, time: {:.2f}min, remaining: {:.2f}min, train_ts: {:.4f}, val_ts: {:.4f}, train_bscore: {:.4f}, val_bscore: {:.4f}'
          .format(epoch + 1, num_epochs, train_loss / sample_size,
                  time_this_epoch_min, time_this_epoch_min * (num_epochs - epoch - 1), train_ts, val_ts, train_bscore, val_bscore))
