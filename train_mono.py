...
from model import MonoEncoder, MonoDecoder, UnetDecoder
import utils
...
for epoch in range(num_epochs):
        ...
        if batch % 10 == 0:
            print('batch [{}], epoch [{}], loss: {:.4f}, \
                  ...
                  .format(batch + 1, epoch + 1, total_train_loss / sample_size,
                          batch_time, avg_train_ts, avg_dynamic_ts_batch, batch_iou_mean))
            if opt.verbose:
    ...
    val_ts_static, predicted_val_map_st, val_iou, val_ts_dynamic, predicted_val_map_dy = utils.evaluation(models, val_loader, DEVICE, opt.num_dynamic_labels)
    ...
    print('epoch [{}/{}], loss: {:.4f}, time: {:.2f}min, remaining: {:.2f}min, \
        ...
        train_iou: {:.4f}, val_iou: {:.4f}'
          .format(epoch + 1, num_epochs, total_train_loss / sample_size,
                  ...
                  np.average(train_dynamic_iou), val_iou))
...
torch.save({
    ...
    FOLDER_PATH + '/monolayout/roadmapnet_{}_final.pth'.format(timestampStr))
